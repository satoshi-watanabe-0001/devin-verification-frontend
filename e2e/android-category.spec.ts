...
test.describe('DEVIN-8: Androidカテゴリページ閲覧', () => {
  ...
  test('8-8: Android製品の製造元表示確認', async ({ page }) => {
    const manufacturers = ['Samsung', 'Sony', 'Google', 'Sharp'];
    let foundManufacturer = false;
    
    for (const manufacturer of manufacturers) {
      const manufacturerText = page.locator(`text=${manufacturer}`);
      const count = await manufacturerText.count();
      if (count > 0) {
    ...
    expect(foundManufacturer).toBe(true);
  });
});
